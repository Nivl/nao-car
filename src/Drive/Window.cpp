...
Window::Window(boost::shared_ptr<AL::ALBroker> broker) :
  QWidget(NULL),
  // _poseManager(broker), _steeringWheelIsTaken(false),
  ...
  _cameraLabel(new QLabel()), _cameraImage(), _cameraPixmap()
{
  ...
  _cameraId = _camera.subscribe(_cameraId, AL::kQVGA, AL::kRGBColorSpace, 30);
...
void Window::updateCamera()
{
  ...
  if (imageData != NULL)
    {
      _cameraImage = QImage(imageData, 320, 240, QImage::Format_RGB888);
