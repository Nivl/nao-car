 ...
 */
var colorsSave = [0x265897, 0x13acfa, 0x009a49, 0xc0000b, 0xa1cb50, 0xB11B23,
              0xFDD901, 0x000000, 0xE44D26, 0x70A3D6, 0x9DC3E6, 0xAAAAAA,
              0xCCCCCC, 0xEEEEEE];
var PI2 = Math.PI * 2;
...
function CountdownMaterial(opts) {
  ...
  this.material = new THREE.ParticleCanvasMaterial({
    ...
    program: this.plainCircle
  });
}

/**
 * Default function used to draw a dot: a plain circle
 * @param {Object} ctx Canvas context to draw on.
 */
CountdownMaterial.prototype.plainCircle = function(ctx) {
  ctx.beginPath();
...
/**
 * Function used to draw a gradient circle
 * @param {Object} ctx Canvas context to draw on.
 */
CountdownMaterial.prototype.gradientCircle = function(ctx) {
	  ctx.beginPath();
	  ctx.arc(0, 0, 1, 0, PI2, true);
	  ctx.closePath();
		var gradient = ctx.createRadialGradient( 0, 0, 0, 0, 0, 1 );
		var r = Math.round(this.color.r*255);
		var v = Math.round(this.color.g*255);
		var b = Math.round(this.color.b*255); 
		var stop0 = 'rgba('+r+','+v+','+b+',1)';
		var stop1 = 'rgba('+r+','+v+','+b+',1)';
		var stop2 = 'rgba('+(r-00)+','+(v-00)+','+(b-00)+',0.1)';
		gradient.addColorStop( 0, stop0 );
	    gradient.addColorStop( 0.2, stop1 );
		//gradient.addColorStop( 0.8, stop2 );
...
CountdownEntity.prototype.build = function() {
  var incr = this.entity.grid.incr;
  this.gridSizeX = incr * this.entity.grid.sizeX;
  this.gridSizeY = incr * this.entity.grid.sizeY;
  for (var i = 0; i < colors.length; i++) {
	var cols = colors[i];
    this.materials.push(new CountdownMaterial({ color: cols[0], size: 30, opacity: cols[1]}));
  }
  
 /* {"swapTime":3000,:0,*/
  
  var xPercent = this.entity.xPos; /* Position finale par rapport à l'écran */ 
  var yPercent = this.entity.yPos; /* Position finale par rapport à l'écran */
  var sc = this.entity.grid.scale;
  for (var i = 0; i < this.entity.dots.length; i++) {
    ...
    if (this.first) {
      var temp = {x: (x * incr) - (this.gridSizeX / 2) + ((xPercent * windowDimensions[0])/100),
                  y: ((y * incr) * -1) + (this.gridSizeY / 2)  + ((yPercent * windowDimensions[1])/100),
                  z: 0};
    } else {
    ...
    this.addParticle(new CountdownParticle({
      originX: (x * incr) - (this.gridSizeX / 2)  + ((xPercent * windowDimensions[0])/100),
      originY: ((y * incr) * -1) + (this.gridSizeY / 2)  - ((yPercent * windowDimensions[1])/100),
      originZ: 0,
      x: temp.x,
      y: temp.y,
      z: temp.z,
      scale: sc, /* Taille des points - En fait le radius */
...
var entitiesLoader = (function() {
  ...
  /** 
   * the callback to call when the last entity has been exploded
   */
  ...
  function swapEntity() {
    ...
    if (currentEntityDesign >= entityDesigns.length) {
    	if (endCallback) {
    		entities[0].exit();  
    		endCallback();
    	}
    }
    // Not the last entity or there's a callback which means that there's more animation
    else {
      showEntity();
      var swapTime = entities[1].entity.swapTime;    
      if (camera.position.z < 800) {
        direction = 1;
      } else if (camera.position.z > 1000) {
        direction = -1;
      }
      // Explode the entity only if it is not the last one OR if there's a callback
      if ((currentEntityDesign + 1 < entityDesigns.length) || (endCallback)) {
        swapTimeout = new TWEEN.Tween(camera.position)
        .to({z: camera.position.z+(200*direction)}, swapTime)
        .onComplete(function() {
          entities[0].exit();
          splodeEntity();
        }).easing(TWEEN.Easing.Circular.EaseIn).start();
  ...
  /**
   * Load dots and colors files and finally launch the animation
   * @param dotsname The dots filename
   * @param colorsname The colors filename
   * @param ccb The callback to call when the entity has been exploded
   */
  var watchAgainTitle = "Watch it again";
  var finalMsg = "";
  var finalUrl = "http://www.yahoo.fr";
  function load(watitle, fm, fu, dotsname, colorsname, cp) {
	  watchAgainTitle = watitle;
	  finalMsg = fm;
	  finalUrl = fu;
	currentEntityDesign = 0;
	endCallback = cp;
	io.injectScripts([dotsname],
      function() {
        io.injectScript(colorsname,
      	  function() {
        	init();
          });
      });
